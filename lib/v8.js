...
const {
  ...
  collectCppHeapStatistics: _collectCppHeapStatistics,
} = binding;
...
function collectCppHeapStatistics(type = 'detailed') {
  if (type !== undefined && type !== 'detailed' && type !== 'brief') {
    throw new ERR_INVALID_ARG_TYPE('Detail type', 'brief, detailed or undefined', type);
  }
  const detailLevel = type === 'brief' ? 0 : 1;
  return _collectCppHeapStatistics(detailLevel);
}
